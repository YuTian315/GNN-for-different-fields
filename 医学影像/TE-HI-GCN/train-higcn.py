...
import numpy as np
import torch
from hinets.model_higcn import MyHIgcn as HiGCN
...
def train(name, device='cuda'):
    ts_folder = StratifiedKFold(n_splits=10, random_state=0, shuffle=True).split(graph_adj, labels)
    ts_result = []
    ts = 0
    ...
    for t_idx, s_idx in ts_folder:
        ...
        best_acc = 0
        train_val_graph_adj = graph_adj[t_idx]
        train_val_labels = labels[t_idx]

        tv_folder = StratifiedKFold(n_splits=10, random_state=0, shuffle=True).split(train_val_graph_adj, train_val_labels)
        ...
        train_node_adj, train_val_node_adj, train_test_node_adj = get_node_adj(train, train_val, train_test, node_adj_data)

        model = HiGCN(in_dim=5, hidden_dim=5, graph_adj=graph_adj, num=ts, thr=0, name=name, brain_region_num=brain_region_num)
        ts += 1
        model = model.to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

        ignored_params = list(map(id, model.gnn.parameters()))
        ...
        CosineLR1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer1, T_max=100, eta_min=0.00001)
        CosineLR2 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer2, T_max=100, eta_min=0.00001)
        ...
        for epoch in range(2000):
            ...
            train_result = {
                ...
                'F1': metrics.f1_score(labels[batch_nodes], train_out.data.cpu().numpy().argmax(axis=1),
                                       average="macro"),
                'auc': metrics.roc_auc_score(labels[batch_nodes], train_out.cpu().data.numpy().argmax(axis=1),
            ...
            with torch.no_grad():
                ...
                torch.save(model.state_dict(), 'checkpointhi.pt')
                torch.save(model.state_dict(), 'model/higcn/ASD_' + name + '/higcn_' + str(ts) + '_0.0' + '.pt')
                print('te epoch', ts, epoch)

        model.load_state_dict(torch.load('model/higcn/ASD_' + name + '/higcn_' + str(ts) + '_0.0' + '.pt'))
        ...
        test_result = {'prec': metrics.precision_score(labels[test], test_output),
                       ...
                       'matrix': metrics.confusion_matrix(labels[test], test_output)
                       }
...
if __name__ == "__main__":
    ...
    pcc, len_pcc = load_correlation(name)
    labels = load_label(name)
    brain_region_num = pcc.shape[1]

    graph_adj = cal_pcc(thr, pcc, len_pcc)
    node_adj_data = load_graph_kernel(len_pcc, name)
