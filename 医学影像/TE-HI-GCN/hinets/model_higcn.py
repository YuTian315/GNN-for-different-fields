...
class MyHIgcn(nn.Module):
    def __init__(self, in_dim, hidden_dim, graph_adj, num, thr=0, num_feats=288, num_nodes=871, num_classes=2,
        ...
        self.gnn = model_gnn(in_dim=in_dim, hidden_dim=hidden_dim, out_dim=num_classes, brain_region_num=self.brain_region_num).to(device)
        self.gnn.load_state_dict(torch.load('model/fgcn/ASD_' + self.name + '/fgcn' + str(self.num) + '_0.' + str(self.thr) + '.pth'))
