...
gender_adj, age_adj, updrs_adj, mixed_adj, all_labels, one_hot_labels, node_weights, dense_features = load_ppmi_data(sparsity_threshold)
adj_dict = {'gender': gender_adj, 'age': age_adj, 'updrs': updrs_adj, 'mixed': mixed_adj}
num_class = 2
num_features_selected = 500
...
def create_support_placeholder(model_name, num_supports, adj):
    ...
    placeholders = {
        ...
        'features': tf.sparse_placeholder(tf.float32, shape=tf.constant([dense_features.shape[0], num_features_selected], dtype=tf.int64)),
...
def train_k_fold(model_name, support, placeholders, is_pool=False, is_skip_connection=True,
    ...
    if model_name == 'res_gcn_cheby':
        model = ResGCN(placeholders, input_dim=num_features_selected, logging=logging, locality_sizes=locality_sizes,
                       is_pool=is_pool, is_skip_connection=is_skip_connection)

    elif model_name == 'gcn':
        model = GCN(placeholders, input_dim=num_features_selected, logging=logging)

    elif model_name == 'gcn_cheby':
        locality = [locality1, locality2]  # locality sizes of different blocks
        model = GCN(placeholders, input_dim=num_features_selected, logging=logging, is_simple=True,
                    is_skip_connection=is_skip_connection, locality=locality)

    elif model_name == 'dense':
        model = MLP(placeholders, input_dim=num_features_selected, logging=logging)
    ...
    for fold in range(num_folds):
        ...
        # feature selection
        features_selected = feature_selection(dense_features, all_labels, labeled_ind, num_features_selected)
        ...
        for epoch in range(FLAGS.epochs):
            ...
            feed_dict = construct_feed_dict(features_selected, support, y_train, train_mask, placeholders)
            ...
            val_cost, val_acc, val_summary, duration = evaluate(features_selected, support, y_val, val_mask, placeholders)
            cost_val.append(val_cost)
            val_writer.add_summary(val_summary, epoch)

            # Evaluation on test set
            test_cost, test_acc, test_summary, test_duration = evaluate(features_selected, support, y_test, test_mask, placeholders)
        ...
        feed_dict = dict()
        feed_dict.update({placeholders['features']: features_selected})##...........Feature_modi
        feed_dict.update({placeholders['support'][i]: support[i] for i in range(len(support))})
        feed_dict.update({placeholders['num_features_nonzero']: features_selected[1].shape})##...........Feature_modi
